...
main()
{
    ...
    do {
        ...
        key=ntru_kem_keypair(pk,sk);
              if (key!=0) {
                fprintf(fp_rsp,"key generation failed\n");
        return KAT_CRYPTO_FAILURE;
       }
        ...
        if ((key=ntru_kem_enc(ct,ss,pk))==0) {
            fprintf(fp_rsp,"encryption failed\n");
            return KAT_CRYPTO_FAILURE;
        }
        ...
        if(ntru_kem_dec(ss,ct,sk,pk) != 0){
            fprintf(fp_rsp,"decryption failed\n");
        }
